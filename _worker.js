...
const UTILITY_NOINDEX_EXACT_PATHS = new Map([
  ["/archive", "archive-search-navigation-utility"],
...
function routePolicyForPath(pathname) {
  ...
  if (path.startsWith("/news/phang-docket-watch/events/")) return routePolicy(AD_ROUTE_STATUS.exclude, "source-docket-event-news-ad-free", "index,follow");
...
function applyRoutePolicyHeaders(headers, pathname) {
  ...
  if (/noindex/i.test(policy.indexability)) headers.set("X-Robots-Tag", policy.indexability);
  return headers;
}
...
<head>
  ...
  <meta name="description" content="Grok Archive Hub topic guide explaining which investigations are ready for indexing, how topic evidence is admitted, and where source-specific records remain provisional.">
...
<main class="page-shell prose">
  ...
  <section>
    ...
    <article>
      <h3>Epstein Death Investigation Records</h3>
      <p>The death-investigation lane starts with the final-48-hours timeline and its supporting source cards. The key public pages are the MCC final-48-hours source chain, the DOJ OIG backbone explainer, the autopsy exhibit-list limits page, the CH0080 camera-file-window analysis, the late-July watch-status review, and the open receipt slots ledger. These pages are designed to answer narrower questions: what a record directly establishes, what adjacent records can corroborate, and which gaps remain open. The protected archive dossiers for EFTA00035147, EFTA00039025, EFTA00039356, EFTA00039416, and EFTA00039660 stay indexed because they now carry document-specific context instead of functioning as bare source cards.</p>
      <p><a href="/research/epstein-final-48-hours-mcc">Open the source-led timeline</a> · <a href="/research/evidence/epstein-death">Open the evidence proof layer</a></p>
    </article>
    <article>
      <h3>Trump In The Epstein Files</h3>
      <p>This lane is limited to named, dated, source-visible records and avoids identity or conduct claims that are not supported by the record being discussed. The topic hub separates the main investigation from the timeline, source map, people-and-roles index, locations, and contradiction ledger. The purpose is to let readers check what the files actually say without blending source appearance, social proximity, legal relevance, and allegation into one category.</p>
    ...
    <article>
      <h3>Banking Records</h3>
      <p>The Banking Records lane is source-first and intentionally restrictive. The public hub remains indexable because it explains the review model, but generated entity pages are excluded unless an entity is verified as a person, organization, trust, institution, property-holding entity, aircraft-owning entity, or other attributable legal entity with real document-specific context. Account endings, address fragments, customer identifiers, product names, OCR debris, subtotal labels, and malformed strings are not promoted as topic pages.</p>
      <p><a href="/banking-records">Open Banking Records</a></p>
    </article>
    <article>
      <h3>Barak Source-Map And Receipts</h3>
      <p>The Barak lane distinguishes evidence of a name appearing in records from evidence of attendance, purpose, intent, relationship, or legal conclusion. Its public pages are structured around source-map navigation, receipt rows, timeline placement, entity-control context, and FARA review signals. Open slots remain marked as open until a source chain supports narrower use.</p>
      <p><a href="/barak/source-map">Open the source map</a> · <a href="/investigations/barak-receipts-presence-not-conduct">Read the presence-not-conduct guide</a></p>
    </article>
  </section>
  ...
  return new Response(enhanceHtmlText(body, request, {
    ...
    description: "Grok Archive Hub topic guide explaining which investigations are ready for indexing, how topic evidence is admitted, and where source-specific records remain provisional."
...
async function phangDocketSitemapEntries(request, env) {
  ...
  try {
    ...
    return [
      ["https://grokarchivehub.com/news", "2026-07-18"],
      ["https://grokarchivehub.com/news/phang-docket-watch", "2026-07-18"]
    ];
  }
}
...
function ensureGlobalExperienceShell(body) {
  let html = body;
  html = html.replace(/\/frontdoor\/site\.css(?:\?[^"' ]*)?/gi, "/frontdoor/site.css?v=GAH-PRESENCE-002");
  html = html.replace(/\/frontdoor\/site\.js(?:\?[^"' ]*)?/gi, "/frontdoor/site.js?v=GAH-PRESENCE-002");
  ...
  if (!/<script\b[^>]*src=["']\/frontdoor\/site\.js/i.test(html)) {
    html = html.replace(/<\/body>/i, `  <script data-cfasync="false" src="/frontdoor/site.js?v=GAH-VISUAL-001"></script>\n</body>`);
  }
  return html;
}
...
function enhanceHtmlText(html, requestOrUrl, meta = {}, env = {}) {
  ...
  const robots = meta.robots || (/noindex/i.test(policy.indexability) ? policy.indexability : "");
  ...
  body = ensureGlobalExperienceShell(body);
...
async function phangAllData(request, env) {
  const staticData = await phangStaticData(request, env);
  const kvEvents = await phangListKvJson(env, PHANG_EVENT_PREFIX);
  const kvEditorial = await phangListKvJson(env, PHANG_EDITORIAL_PREFIX);
  const kvMatches = await phangListKvJson(env, PHANG_MATCH_PREFIX, 1000);
  const kvCorrections = await phangListKvJson(env, PHANG_CORRECTION_PREFIX);
...
async function servePhangEventPage(request, env, id) {
  ...
  return phangHtmlResponse(request, env, phangPageShell({
    ...
    title: `${editorial.headline} | Grok Archive Hub`,
...
function xSafeReturnPath(value) {
  ...
  if (raw === "/admin/x-publisher" || raw === "/admin/x-diagnostics" || raw === "/admin/ai" || raw === "/admin/phang-docket-review") return raw;
  if (raw.startsWith("/admin/x-publisher?") || raw.startsWith("/admin/x-diagnostics?") || raw.startsWith("/admin/ai?") || raw.startsWith("/admin/phang-docket-review?")) return raw;
  if (raw.startsWith("/admin/x-publisher#") || raw.startsWith("/admin/x-diagnostics#") || raw.startsWith("/admin/ai#") || raw.startsWith("/admin/phang-docket-review#")) return raw;
